...
class ConfigurablePlaceOrderTest extends BaseCase
{
    public $product_url = "/mars-heattech-trade-pullover.html";
    public $product_title = "Mars HeatTech";
    ...
    public function test()
    {
        ...
        //set and trigger configurable attributes: Size
        $set_val = array('script' => "window.jQuery('#attribute152').val('195');", 'args' => array());
        $trig_val = array('script' => "window.jQuery('#attribute152').trigger('change');", 'args' => array());
        $this->execute($set_val);
        $this->execute($trig_val);
        sleep($this->js_delay);

        //check configurable attribute: Size
        $get_val = array('script' => "return window.jQuery('#attribute152').val();", 'args' => array());
        $this->assertEquals('195',$this->execute($get_val));

        //set configurable attributes: Color
        $set_val = array('script' => "window.jQuery('#attribute90').val('69');", 'args' => array());
        $this->execute($set_val);

        //check configurable attribute: Color
        $get_val = array('script' => "return window.jQuery('#attribute90').val();", 'args' => array());
        $this->assertEquals('69',$this->execute($get_val));

        //click add to cart: js implementation
        //$this->execute(array('script' => "window.jQuery('#product-addtocart-button').trigger('click');", 'args' => array()));
        //sleep($this->js_delay);
    ...
    private function Shopping_Cart()
    {
       ...
       //check product in cart
        $this->assertContains('Mars HeatTech',$this->byXPath('//*[@id="shopping-cart-table"]/tbody/tr[1]/td[1]/div/strong/a')->text());
